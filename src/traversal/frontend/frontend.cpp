...
static double bench_gpu(Bvh2* bvh2, RayAoS* rays, HitAoS* hits, size_t n) {
    auto t0 = anydsl_get_kernel_time();
    frontend_gpu_traverse_bvh(bvh2, rays, hits, n);
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < warmup; i++) {
        if (use_gpu) bench_gpu(&bvh2, rays.data(), hits.data(), rays.size());
        else         bench_cpu(&bvh4, rays.data(), hits.data(), rays.size());
    }

    std::vector<double> timings;
    for (int i = 0; i < iters; i++) {
        auto t = use_gpu
            ? bench_gpu(&bvh2, rays.data(), hits.data(), rays.size())
