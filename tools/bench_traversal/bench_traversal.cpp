...
inline void usage() {
    std::cout << "Usage: bench_traversal [options]\n"
                 ...
                 "  -s       --single          Use single rays on the CPU (disabled by default)\n"
...
int main(int argc, char** argv) {
    ...
    if (use_gpu && single_ray) {
        std::cerr << "Single ray tracing is only available on the CPU" << std::endl;
        return 1;
    }
    ...
    else if (single_ray) bench = [&] { return bench_cpu(&bvh4, rays1.data(), hits1.data(), ray_count, any_hit); };
    else                 bench = [&] { return bench_cpu(&bvh4, rays8.data(), hits8.data(), ray_count, any_hit); };
