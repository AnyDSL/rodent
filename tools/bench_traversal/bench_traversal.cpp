...
static double bench_cpu_hybrid(Node8* nodes, Tri4* tris, Ray8* rays, Hit8* hits, size_t n, bool any_hit) {
    ...
    if (any_hit) cpu_occluded_hybrid8_bvh8_tri4_avx2(nodes, tris, rays, hits, n);
    else         cpu_intersect_hybrid8_bvh8_tri4_avx2(nodes, tris, rays, hits, n);
...
static double bench_cpu_packet(Node8* nodes, Tri4* tris, Ray8* rays, Hit8* hits, size_t n, bool any_hit) {
    auto t0 = anydsl_get_micro_time();
    if (any_hit) cpu_occluded_packet8_bvh8_tri4_avx2(nodes, tris, rays, hits, n);
    else         cpu_intersect_packet8_bvh8_tri4_avx2(nodes, tris, rays, hits, n);
    auto t1 = anydsl_get_micro_time();
    return (t1 - t0) / 1000.0;
}
...
static double bench_cpu_single(Node8* nodes, Tri4* tris, Ray1* rays, Hit1* hits, size_t n, bool any_hit) {
    auto t0 = anydsl_get_micro_time();
    if (any_hit) cpu_occluded_single_bvh8_tri4_avx2(nodes, tris, rays, hits, n);
    else         cpu_intersect_single_bvh8_tri4_avx2(nodes, tris, rays, hits, n);
    auto t1 = anydsl_get_micro_time();
    return (t1 - t0) / 1000.0;
}
...
static double bench_cpu_hybrid(Node4* nodes, Tri4* tris, Ray8* rays, Hit8* hits, size_t n, bool any_hit) {
    auto t0 = anydsl_get_micro_time();
    if (any_hit) cpu_occluded_hybrid8_bvh4_tri4_avx2(nodes, tris, rays, hits, n);
    else         cpu_intersect_hybrid8_bvh4_tri4_avx2(nodes, tris, rays, hits, n);
    auto t1 = anydsl_get_micro_time();
    return (t1 - t0) / 1000.0;
}
...
static double bench_cpu_packet(Node4* nodes, Tri4* tris, Ray8* rays, Hit8* hits, size_t n, bool any_hit) {
    auto t0 = anydsl_get_micro_time();
    if (any_hit) cpu_occluded_packet8_bvh4_tri4_avx2(nodes, tris, rays, hits, n);
    else         cpu_intersect_packet8_bvh4_tri4_avx2(nodes, tris, rays, hits, n);
    auto t1 = anydsl_get_micro_time();
    return (t1 - t0) / 1000.0;
}
...
static double bench_cpu_single(Node4* nodes, Tri4* tris, Ray1* rays, Hit1* hits, size_t n, bool any_hit) {
    auto t0 = anydsl_get_micro_time();
    if (any_hit) cpu_occluded_single_bvh4_tri4_avx2(nodes, tris, rays, hits, n);
    else         cpu_intersect_single_bvh4_tri4_avx2(nodes, tris, rays, hits, n);
    auto t1 = anydsl_get_micro_time();
    return (t1 - t0) / 1000.0;
}
...
static double bench_gpu(Node2* nodes, Tri1* tris, Ray1* rays, Hit1* hits, size_t n, bool any_hit) {
    ...
    if (any_hit) gpu_occluded_single_bvh2_tri1_nvvm(nodes, tris, rays, hits, n);
    else         gpu_intersect_single_bvh2_tri1_nvvm(nodes, tris, rays, hits, n);
...
int main(int argc, char** argv) {
    ...
    if (use_gpu) bench = [&] { return bench_gpu(nodes2.data(), tris1.data(), rays1.data(), hits1.data(), ray_count, any_hit); };
        ...
        if (bvh_width == 4) {
            ...
            else if (packet) bench = [&] { return bench_cpu_packet(nodes4.data(), tris4.data(), rays8.data(), hits8.data(), rays8.size(), any_hit); };
            ...
            else if (packet) bench = [&] { return bench_cpu_packet(nodes8.data(), tris4.data(), rays8.data(), hits8.data(), rays8.size(), any_hit); };
